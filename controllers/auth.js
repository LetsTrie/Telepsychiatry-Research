...
exports.verifyAccount = async(req, res, next) => {
    await eUserModel.updateOne({ _id: req.params.id }, { $set: { isVerified: true } });
    req.flash(
        'successMessage',
...
exports.updateExpertFile = (req, res, next) => {
    ...
    const filePath =
        '/home/mah-nigga/NewTrin/xTrin/public/uploads/' + req.user.propicURL;
    fs.unlink(filePath, function(err) {
        const filename = req.body.filename + '-' + req.file.originalname;
        eUserModel.findOneAndUpdate({ _id: req.user._id }, {
                $set: {
                    propicURL: filename,
                },
            },
...
exports.postUpdateExpUser = async(req, res, next) => {
    ...
    const hashed = await bcrypt.hash(req.body.password, 10);
    await eUserModel.findOneAndUpdate({ _id: req.user._id }, {
        $set: {
            name: fullName,
