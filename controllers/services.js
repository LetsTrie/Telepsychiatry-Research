...
exports.dateTimeReset = async (req, res) => {
  ...
  sendSMTP(email, date, time, doctor, id);
  res.redirect('back');
};
...
const sendSMTP = async (email, date, time, doctor, id) => {
  const mailBody = `Your meeting with ${doctor} has been shifted to ${date} at ${time}<br>This is your <a href="https://media.monerdaktar.com/${id}" target="_blank">Meeting Link</a>`;
  let mailOptions = {
